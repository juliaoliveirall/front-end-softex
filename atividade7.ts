...
function calcularDesconto(valorProduto, desconto) {
    let percentualDesconto = (valorProduto * desconto)/ 100;
...
// questão 16
// questão 17 
function verificarPropriedade(obj: object, prop: string) {
...
function palavraLonga(frase) {
    ...
    let palavraLonga = palavras.reduce((longest, current) => {
...
// questão 20
// questão 21
function stringsLongas(strings: string[]): string[] {
    return strings.filter(str => str.length > 5);
}
let arrayStrings = ['maçã', 'banana', 'tangerina', 'manga'];
